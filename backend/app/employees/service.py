    ...
    if existing_code is not None:
        raise HTTPException(
            ...
            detail={
                ...
                "message": f"Employee code '{payload.employee_code}' already exists.",
            },
        )
...
def get_employees(db: Session, page: int = 1, page_size: int = 20) -> tuple[list[Employee], int]:
    if page < 1:
        page = 1
    if page_size < 1:
        page_size = 20
    ...
    employees = db.execute(
        select(Employee).offset(offset).limit(page_size)
    ).scalars().all()

    return list(employees), total


def get_employee_by_id(db: Session, employee_id: uuid.UUID) -> Employee:
    employee = db.execute(
    ...
    if "employee_code" in update_data:
        ...
        if existing_code is not None:
            raise HTTPException(
                ...
                detail={
                    "code": "DUPLICATE_EMPLOYEE_CODE",
                    "message": f"Employee code '{update_data['employee_code']}' already exists.",
                },
            )
