...
All responses wrapped in the shared {"success": ..., "data"/"error": ...}
envelope. Service exceptions mapped to HTTP status codes here.
"""
...
@router.get("/payruns")
def api_list_payruns(db: Session = Depends(get_db)):
    payruns = list_payruns(db)
    return _ok([PayrunResponse.model_validate(p).model_dump(mode="json") for p in payruns])


@router.post("/payruns")
def api_create_payrun(payload: CreatePayrunRequest, db: Session = Depends(get_db)):
    try:
        payrun = create_payrun(db, payload)
    except PayrollServiceError as exc:
        return _error(exc)
    return _ok(PayrunResponse.model_validate(payrun).model_dump(mode="json"))
...
def api_compute_payrun(
    ...
    employee_ids = payload.employee_ids if payload else []
    worked_days_overrides = payload.worked_days_overrides if payload else None
    try:
        payrun, payslip_count, warnings = compute_payrun(
            db, payrun_id, employee_ids, hr_client, worked_days_overrides
        )
    except PayrollServiceError as exc:
        return _error(exc)
    return _ok(
...
def api_validate_payrun(payrun_id: UUID, db: Session = Depends(get_db)):
    try:
...
def api_mark_payrun_paid(payrun_id: UUID, db: Session = Depends(get_db)):
    try:
...
def api_get_payslip(payslip_id: UUID, db: Session = Depends(get_db)):
    try:
    ...
    detail = PayslipDetailResponse(
        ...
        payrun=PayrunStub(id=payrun.id, name=payrun.name, status=payrun.status),
        salary_structure=SalaryStructureStub(id=structure.id, name=structure.name),
        period_start=payrun.period_start,
        period_end=payrun.period_end,
        worked_days=payslip.worked_days,
        status=payslip.status,
        salary_breakdown=[PayslipLineResponse.model_validate(l) for l in lines],
...
@router.get("/payslips")
def api_list_payslips(payrun_id: UUID = Query(...), db: Session = Depends(get_db)):
    payslips = list_payslips_for_payrun(db, payrun_id)
    return _ok(
        [PayslipSummaryResponse.model_validate(p).model_dump(mode="json") for p in payslips]
    )